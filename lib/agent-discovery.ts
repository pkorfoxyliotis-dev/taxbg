...
export async function buildAgentSiteTree(): Promise<AgentSiteTree> {
  ...
  return {
    ...
    discovery: [
      ...
      { order: 4, path: "/agent-info.json", type: "agent_disclosure", note: "public — NOT under /agent/" },
      { order: 5, path: "/agent/site-tree.json", type: "machine_tree" },
      { order: 6, path: "/agent/registry.json", type: "registry" },
      { order: 7, path: "/agent/trust.json", type: "trust" },
      { order: 8, path: "/agent/capabilities.json", type: "capabilities" },
      { order: 9, path: "/agent/guide.json", type: "guide_tree" },
      { order: 10, path: "/agent/services-catalog.json", type: "catalog", note: "problem/why/solution per service, autoupdates from content" },
    ],
    paths: {
      core,
...
export function formatLlmsIndex(tree: AgentSiteTree): string {
  ...
  return [
    ...
    "",
    "## Ανακάλυψη (σειρά — identity, agent-info & llms ΔΗΜΟΣΙΑ, όχι μέσα στο /agent/)",
    "1. /llms.txt (αυτό)",
    "2. /llms_full.txt",
    "3. /identity.json (δημόσιο identity)",
    "4. /agent-info.json (δημόσια AI αποκάλυψη / digital handshake)",
    "5. /agent/site-tree.json",
    "6. /agent/registry.json (νομικά στοιχεία, UIC)",
    "7. /agent/trust.json",
    "8. /agent/capabilities.json",
    "9. /agent/guide.json",
    "10. /agent/services-catalog.json (πρόβλημα→γιατί→λύση ανά υπηρεσία)",
    "",
    "## Κύριες υπηρεσίες",
    svc,
    "",
...
export function formatLlmsFull(tree: AgentSiteTree): string {
  return [
    ...
    "| AI disclosure | /agent-info.json | Δημόσιο, root — ΟΧΙ /agent/ |",
    "| Site tree | /agent/site-tree.json | Machine |",
    "| Registry | /agent/registry.json | Machine — νομικά στοιχεία |",
    "| Trust | /agent/trust.json | Machine |",
    "| Capabilities | /agent/capabilities.json | Machine |",
    "| Guide | /agent/guide.json | Machine |",
    "| Services catalog | /agent/services-catalog.json | Machine — πρόβλημα→γιατί→λύση, autoupdate |",
    "| Chat | POST /api/agent/chat | Proxy n8n |",
    "",
    "## Υπηρεσίες & τιμές (πρόβλημα → γιατί → λύση)",
    ...services.flatMap((s) => [
      `### ${s.titleEl}`,
      `**Πρόβλημα:** ${s.problemEl}`,
      `**Γιατί:** ${s.whyEl}`,
      `**Λύση:** ${s.solutionEl}`,
      ...s.tiers.map(
        (t) =>
